...
public class KSPBasedOnLength {


	int x=VirtualHopsParameter.BP_X;
	public List<List<Node>> kspYenWithoutLoops(String boltID,int startNodeId, int endNodeId, Map<Node, List<Node>> graph, Map<Points,Double> edgeSet,Map<Points,Map<String,Boolean>> yenFlagMap) //TimeThread tt)
	{
	...
	//	 long startTime = System.currentTimeMillis();
		...
		//	PrintFunctions.printPath(baseLinePath, "baseLinePath");
			...
			canidatePathList.remove(potentialPath);
			 if(!lengthList.contains(potentialPath.size()))
			 {
				 lengthList.add(potentialPath.size());
			 }
			 if(lengthList.size()<x||lengthList.size()==x)
			 {
				 baseLinePathQueue.offer(potentialPath);
				 kShortestPathList.add(potentialPath);
			 }
		}
		//createYenFlagMap(boltID,yenFlagMap);
		//System.out.println("KSP Number of paths: "+kShortestPathList.size());
		//PrintFunctions.printKSP(kShortestPathList);
